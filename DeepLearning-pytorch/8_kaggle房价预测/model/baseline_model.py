...
'''数据集预处理'''
def data_pro():
    train_path="8_kaggle房价预测\dataset\\train.csv"
    test_path="8_kaggle房价预测\dataset\\test.csv"
...
if __name__=="__main__":
    args=parse_args()
    #数据集预处理
    in_features,train_f,train_label,test_f,test_data=data_pro()
    #模型
    ...
    with open("8_kaggle房价预测\log\log.txt",mode="a",encoding="utf-8")as f:
        f.write(f'{args.k}-折验证: 平均训练log rmse: {float(train_l):f}, '
                f'平均验证log rmse: {float(valid_l):f},\n'
                f"参数:lr={args.lr},w_decay={args.w_decay},num_epochs={args.num_epochs},batch_size={args.batch_size},k={args.k}\n")
